...
public class FrequencyOfElement {
    ...
    public static void main(String[] args) {  //main method
        ...
        int[] arr = {1,1,1,11,1,2,2,3,3,3,4,4,1,1,1,1}; // 8 kere 1 elementi tekrarlanmış
        int n = frequencyOfElement(arr, 1);
        System.out.println(n);
    }
                                 //    [1,1,2,3,4,5] 1 kaç kere tekrarlanmış
    //returns the frequency of the given element from the given array
    public static int frequencyOfElement(int[] array, int element){ // return type oluyo static int olduğu için
        int count = 0;
        for (int each : array) {
            if(each == element){
                count++;
